...
// --- CONFIGURATION ---
const MONGODB_URI =
  'mongodb+srv://Kruti98:Kruti98.@cluster0.lkh2x.mongodb.net/epms_new';
  ...
  try {
    ...
    for (const name of ORG_NAMES) {
      const org = await Organization.create({
        name,
        slug: slugify(name),
        subtitle: `Leadership in ${name}`,
        primaryColor: '#6366f1',
        secondaryColor: '#4338ca',
        industry: getRandom(['Tech', 'Edu', 'Gov']),
        size: '500+',
        contactEmail: `hi@${slugify(name)}.pro`,
        website: 'https://epms.pro',
        address: 'Global',
      });
      orgs.push(org);
    }
    ...
    for (let globalIndex = 0; globalIndex < 2; globalIndex++) {
      for (const orgIdx of adminsPerGlobal[globalIndex]) {
        ...
        const admin = await User.create({
          email: `admin${globalIndex + 1}_${org.slug}@epms.com`,
          firstName: 'System',
          lastName: 'Admin',
        ...
        for (let m = 0; m < 3; m++) {
          managers.push(
            await User.create({
              email: `manager${m + 1}_${org.slug}@epms.com`,
        ...
        for (let e = 0; e < 20; e++) {
          ...
          employees.push(
            await User.create({
              email: `emp${e + 1}_${org.slug}@epms.com`,
              firstName: getRandom(FIRST_NAMES),
              lastName: getRandom(LAST_NAMES),
              password: hashedPassword,
              role: 'EMPLOYEE',
        ...
        await Payroll.insertMany(
          allUsers.map((u) => ({
            ...
            month: 4,
            year: 2024,
            baseSalary: (u as any).baseSalary,
            netAmount: (u as any).baseSalary,
            status: 'PAID',
            organizationId: org._id,
          })),
        );
        await Asset.insertMany(
          employees.map((e) => ({
            ...
            assignedTo: e._id,
            organizationId: org._id,
            identifier: `SN-${e.email}`,
          })),
        );
        await SocialPost.insertMany([
          { authorId: admin._id, content: 'Welcome!', organizationId: org._id },
        ]);
        ...
        await Poll.insertMany([
          {
            ...
            description:
              'Seeking employee feedback for upcoming policy changes.',
            options: [
              { text: 'Yes, absolutely', count: 15 },
              { text: 'No, let stay at 5', count: 5 },
            ],
